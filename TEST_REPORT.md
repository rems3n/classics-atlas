...
## Reader beta — September 21, 2026

Added 10 paths / 60 assignments, 13 catalog records (1,492 total), account sync, collections, My Atlas, guidance and discovery. New API integration covers registration, authentication requirements, cross-user state isolation, optimistic revision conflicts, origin rejection, private/unlisted/public collection access, ownership of edits/deletions, persistence across server restart, recovery token rotation and account deletion. Existing 26 core tests pass. New real-browser flows pass for path progress, map filtering/reset, private notes, favorites, atlas totals, guest collection creation, sign-up, explicit migration, reload sync, anonymous unlisted sharing, light/dark rendering and mobile overflow. External images are blocked in deterministic UI tests; this release does not claim new verified cover matches.

Unresolved path items stay visible; assignments do not automatically mark complete works read. Production deployment verification follows the same /health and public page checks. SQLite requires the configured persistent Railway volume and one replica.
