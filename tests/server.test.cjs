...
test('page is served with security headers, a strong ETag and 304 revalidation',async()=>{
 let r=await fetch(base+'/');assert.equal(r.status,200);assert.equal(r.headers.get('x-content-type-options'),'nosniff');assert.equal(r.headers.get('x-frame-options'),'DENY');
 const etag=r.headers.get('etag');assert.match(etag,/^"[A-Za-z0-9_-]+"$/);assert((await r.text()).includes('window.ATLAS_DATA='));
 r=await fetch(base+'/',{headers:{'If-None-Match':etag}});assert.equal(r.status,304);assert.equal(await r.text(),'');
 r=await fetch(base+'/',{method:'HEAD'});assert.equal(r.status,200);assert.equal(r.headers.get('etag'),etag);
});

test('compressed variants decode to the same page and are reused',async()=>{
 const http=require('node:http');
 const raw=enc=>new Promise((res,rej)=>http.get(base+'/',{headers:{'Accept-Encoding':enc}},r=>{const c=[];r.on('data',d=>c.push(d));r.on('end',()=>res({h:r.headers,b:Buffer.concat(c)}));}).on('error',rej));
...
test('shareable book and path links get their own title and ETag',async()=>{
 ...
 const home=await fetch(base+'/'),r=await fetch(base+'/?book='+book.id),html=await r.text();
 assert(html.includes('<title>'+book.title.replace(/&/g,'&amp;').replace(/</g,'&lt;')));assert.notEqual(r.headers.get('etag'),home.headers.get('etag'));
 const unknown=await fetch(base+'/?book=not-a-book');assert.equal(unknown.headers.get('etag'),home.headers.get('etag'),'unknown ids fall back to the default page');
});
